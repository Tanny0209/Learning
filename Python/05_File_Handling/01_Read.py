# Purpose: shows the four main ways to read a file in Python
# (read all, read a fixed number of characters, read one line, read all lines as a list).
# The same text file is used throughout, so pay attention to the position (cursor)
# of the file object, which moves forward as we read.

# open() opens the file and returns a "file object" we can read from.
...
# Everything after this reads nothing unless we re-open the file.
